...
export async function sendPush(deviceToken, { title, body, data = {} }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Wait for response headers first, then collect body
    ...
    // If stream closes with no response headers (e.g. GOAWAY / bad JWT)
    req.on('close', () => {
      session.destroy();
      resolve({ ok: false, status: 0, body: 'no response (connection closed)' });
    });
  });
}
